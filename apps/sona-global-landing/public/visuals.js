...
// ============================================
// APERO-STYLE 3D FLOATING CARDS - Featured Products
// ============================================
function initAperoProducts() {
  const container = document.getElementById('apero-products');
  if (!container) return;
  ...
  container.addEventListener('mousemove', (e) => {
    ...
    cards.forEach((card, index) => {
      ...
      const rotateY = mouseX * 15 * depth;
      const rotateX = -mouseY * 15 * depth;
      ...
      card.style.transform = `
        ...
        translateY(${Math.sin(Date.now() / 1000 + index) * 20}px)
      `;
    });
  });
  ...
  function floatCards() {
    ...
    cards.forEach((card, index) => {
      const baseY = Math.sin(time + index * 0.5) * 20;
      const baseRotate = Math.sin(time * 0.3 + index) * 5;
      
      // Only apply base animation if mouse is not hovering
      if (mouseX === 0 && mouseY === 0) {
        card.style.transform = `
          perspective(1500px)
          translateY(${baseY}px)
          rotateZ(${baseRotate}deg)
          scale(${1 + Math.sin(time + index) * 0.05})
        `;
      }
...
// ============================================
// SIGNATURE VISUAL INTERACTIVE 3D TILT
// ============================================
function initSignatureVisuals() {
  const cards = document.querySelectorAll('[data-tilt-sig]');
...
if (document.readyState !== 'loading') {
  ...
  document.addEventListener('DOMContentLoaded', initSignatureVisuals);
}
